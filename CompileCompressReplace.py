...
if len(sys.argv) == 5:
    ...
    bnkPath = sys.argv[2].replace("\"", "")
    LuaCPlusPath = sys.argv[3].replace("\"", "")
    offzipPath = sys.argv[4].replace("\"", "")
else:
...
if(not os.path.isfile(bnkPath)):
	print("Can't find the bnk file to modify! Make sure the path is correct. (2nd arg)")
	sys.exit()
if(not os.path.isfile(LuaCPlusPath)):
	print("Can't find LuaC! Make sure the path is correct. (3rd arg)")
	sys.exit()
if(not os.path.isfile(offzipPath)):
	print("Can't find offzip! Make sure the path is correct. (4th arg)")
	sys.exit()
...
subprocess.run(LuaCPlusPath + " -s -o " + "\"" + compiledPath + "\"" + " " + "\"" + plaintextPath + "\"")
...
subprocess.run(offzipPath + " " + "\"" + bnkPath + ".toc" + "\"" + " \"" + bnkPath + ".tocDecomp\"")
